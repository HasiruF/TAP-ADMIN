...
>
> ⚠️ **AUDIT UPDATE — 2026-07-31:** The premise of this document's §1 header ("tap-admin's own thin `/api/admin/*` BFF routes that today serve mock data") no longer holds at all — those BFF/mock routes have been **deleted from the codebase**, not just "connected to the real backend." There is no `/api/admin/*` route handler and no `BACKEND_API_URL` usage left in `src/`; every wrapper in `src/lib/api/admin/*.ts` calls the listed `/admin/...` (and, for the marketplace, `/vendors/...`) backend paths directly via `NEXT_PUBLIC_API_URL`. Also note: `.env.local` today sets `PUBLIC_API_URL` (no `NEXT_PUBLIC_`/`BACKEND_API_URL` prefix), which is not read by any app code — the real base URL comes from `.env`'s `NEXT_PUBLIC_API_URL`. Most endpoint specs in §2 below still match current usage; a Vendors/marketplace API surface (`/vendors/categories`, `/vendors/listings`, `/vendors/listing-photos`) and an analytics API surface (`/admin/analytics/*`) were added since and are not covered here — see [tap_admin_data_contracts.md](tap_admin_data_contracts.md) §10–11.
